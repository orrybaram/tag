...
class UsereventsController < ApplicationController
  ...
  def create
    @userevent = Userevent.new
    @userevent.user_id = session[:idusers]
    @userevent.event_id = params[:event_id]
    @userevent.level_id = params[:Level][:level_id]

    respond_to do |format|
      if @userevent.save
        format.html { redirect_to '/admin', :notice => 'Usersports preference was successfully created.' }
      end
    end
  end
